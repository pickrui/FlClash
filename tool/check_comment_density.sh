...
comment_lines() {
  ...
  printf '%s\n' "$body" |
    ...
    grep -vE 'ignore:|ignore_for_file:|coverage:ignore|Copyright|SPDX-|Licensed under|GENERATED|dart format off|yaml-language-server|//[[:space:]]*(go:[a-z]+|\+build)|[[:space:]]#!$|[[:space:]]#!/|#!?\[|#(include|define|undef|ifdef|ifndef|if|elif|else|endif|pragma|error)\b' || true
}
...
instead. Keep the few that record a non-obvious constraint, and prefer a test or a docs/
entry when the fact belongs there. See the Comments section of AGENTS.md.
